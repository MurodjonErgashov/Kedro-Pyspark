"""Thin executors that run the inventory bronze/silver/gold notebooks.

The read/transform/write logic lives in `notebooks/inventory/`, not here --
...
def run_raw_to_bronze() -> str:
    run_notebook("inventory", "01_raw_to_bronze.ipynb")
    return "done"


def run_bronze_to_silver(_raw_to_bronze_done: str) -> str:
    run_notebook("inventory", "02_bronze_to_silver.ipynb")
    return "done"


def run_silver_to_gold(_bronze_to_silver_done: str) -> None:
    run_notebook("inventory", "03_silver_to_gold.ipynb")
